...
foreach ( array( 'memory', 'keywords', 'plan', 'issues', 'metrics', 'log', 'vectors', 'outcomes', 'competitors', 'backlinks', 'experiments', 'rank_history', 'tasks', 'graph', 'competitor_velocity', 'usage' ) as $table ) {
	$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}vmsb_{$table}" );
}

foreach ( array( 'vmsb_settings', 'vmsb_db_version', 'vmsb_installed_at', 'vmsb_last_scan', 'vmsb_author_id', 'vmsb_robots_sitemap', 'vmsb_delete_data_on_uninstall', 'vmsb_conversion_forecast', 'vmsb_traffic_forecast', 'vmsb_market_assessment', 'vmsb_health_last_check', 'vmsb_last_daily_run', 'vmsb_last_market_assessment' ) as $option ) {
